...
class Functor f => Eval f where
  andZ :: f Bool -> f Bool -> f Bool
  sumZ :: f Double -> f Double -> f Double
  lessZ :: Ord a => f a -> f a -> f Bool
...
instance (Functor f, Zippable f) => Eval f where
  andZ = zipWith_ (&&)
  sumZ = zipWith_ (+)
...
mapBinary2 opName g v1 v2 = case (v1, v2) of
  (ConstValue a, ConstValue b) -> constV <$> valueDynZip msg (on g unI) a b
  (ConstValue a, DynValue   b) -> dynV <$>  valueDynZip msg (fmap . g . unI) a b
  (DynValue   a, ConstValue b) -> dynV <$> valueDynZip msg (fmap . (flip g) . unI) b a
  (DynValue   a, DynValue   b) -> dynV <$> valueDynZip msg (zipWith_ g) a b
...
mapBinary opName g v1 v2 = case (v1, v2) of
  (ConstValue a, ConstValue b) -> fmap2 constV g <$> fromConstValue a <*> fromConstValue b
  (ConstValue a, DynValue   b) -> fmap dynV . fmap . g  <$> fromConstValue a <*> fromDynValue b
  (DynValue   a, ConstValue b) -> fmap dynV . fmap . flip g <$> fromConstValue b <*> fromDynValue a
  (DynValue   a, DynValue   b) -> fmap dynV . zipWith_ g <$> fromDynValue a <*> fromDynValue b
  _                                 -> throwError $ "'" <> shot opName <> "' not supported on Sets"
{-# INLINABLE mapBinary #-}
...
eval :: forall f m . (Error m, Typeable f, Eval f) => VarValues m f -> DesugaredExpr -> m (Value f)
eval vars expr = case expr of 
  ...
  BinaryE (Base op) e1 e2 -> case op of
            Sum         -> eval2'$ \case
              (NullV, x)                -> pure x
              (x, NullV)                -> pure x
              (NumberD x, NumberD y)    -> pure $ dynV $ sumZ x y 
              _ -> err "uequires two Numbers"
            ...
            --     OneV    -> pure a
            --     _       -> mapBinary @Double op    (/) a b
            And         -> eval2 $ \a b -> case (a, b) of
              (FalseV   , _)            -> pure falseV
              (TrueV    , x)            -> pure x
              (_        , FalseV)       -> pure falseV
              (BoolD x, BoolD y)        -> pure $ dynV $ andZ x y
    ...
    where asd = ()
          mb2 g = do {a <- recurse e1; b <- recurse e2; mapBinary2 op g a b}
          mb2 :: IsDynValue b => (forall a . IsDynValue a => a -> a -> b) -> m (Value f)

          eval2 f = do {a <- recurse e1; b <- recurse e2; f a b}
          eval2' f = do {a <- recurse e1; b <- recurse e2; f (a, b)}

          err msg = throwError $ "'" <> shot op <> "' " <> msg

  where recurse = eval vars
{-# INLINABLE eval #-}

            
parseAndEval :: (Error m, Typeable f, Eval f) => VarValues m f -> String -> m (Value f)
