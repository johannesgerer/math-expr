...
maxBool :: MonadError Text m => V.Vector VarName -> DesugaredExpr -> m DesugaredExpr
maxBool v = fmap2 snd $ boundAbs v $ BoolT True
{-# INLINABLE maxBool #-}

minBool :: MonadError Text m => V.Vector VarName -> DesugaredExpr -> m DesugaredExpr
minBool v = fmap2 snd $ boundAbs v $ BoolT False
{-# INLINABLE minBool #-}

maxAbs :: MonadError Text m => V.Vector VarName -> DesugaredExpr -> m DesugaredExpr
...
boundAbs ::  forall m . MonadError Text m
  => V.Vector VarName -> BoundType -> DesugaredExpr -> m (Bool, DesugaredExpr)
{-# INLINABLE boundAbs #-}
boundAbs vars btype e0 = let
  recurse = boundAbs vars btype
  inverse e = flip (boundAbs vars) e =<< typeInverse btype
  useDeterministicSelf = pure (False, typeAbsE btype e0)
  maxV = pure (True, typeBoundE btype)
  useMaxIf = bool useDeterministicSelf maxV
  useDetSelfOr f a b = do
    (d1,b1) <- a
    (d2,b2) <- b
    if d1 || d2 then (True,) <$> f b1 b2 else useDeterministicSelf
  useMaxIfnonDet1 = useMaxIf . not . deterministic vars
  ...
  in
  case e0 of
    VarE          v -> useMaxIf $ V.elem v vars
    LiteralE      l -> ($ useDeterministicSelf) $ case l of
      BoolL     _ -> withBool
    ...
    UnaryE op e1  -> case op of
      ...
      Iverson     -> case btype of
                       ...
                       BoolT _          -> case e1 of
                                             ...
                                             _       -> useMaxIfnonDet1 e1
                       MaxAbsNonZeroT   -> withNumber $ fmap iversonE <$> boundAbs vars (BoolT True) e1
    BinaryE (Extra op) _ _ -> case op of
    BinaryE (Base op) e1 e2 ->
      let useMaxIfnonDet2 = useMaxIf $ not $ on (&&) (deterministic vars) e1 e2
          fa' g = useDetSelfOr g (recurse e1) $ recurse e2
          fa = fa' . fmap2 pure
      in case op of
      ...
      Quotient          -> withNumber $ bool maxV (fmap (flip quotientE $ absE e2)
                                                   <$> recurse e1)  $ deterministic vars e2 
      ...
      Less              -> useMaxIfnonDet2
      LessEqual         -> useMaxIfnonDet2
      Greater           -> useMaxIfnonDet2
      GreaterEqual      -> useMaxIfnonDet2
      Equal             -> useMaxIfnonDet2
      NotEqual          -> useMaxIfnonDet2
                                         
      Element           -> useMaxIfnonDet2
      NotElement        -> useMaxIfnonDet2     
...
-- | `Nothing` means there is no know expression, the value cannot be determined, it could be anything
deterministic :: V.Vector VarName -> Expr' a -> Bool
deterministic vars = \case
          VarE          v -> not $ V.elem v vars
          LiteralE      _ -> True
          SetE          l -> all (deterministic vars) l
          UnaryE _      e -> deterministic  vars e
          BinaryE _   a b -> on (&&) (deterministic  vars) a b
{-# INLINABLE deterministic #-}
